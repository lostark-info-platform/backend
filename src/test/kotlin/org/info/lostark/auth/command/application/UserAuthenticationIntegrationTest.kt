...
class UserAuthenticationIntegrationTest(
    ...
    Given("유저정보가 없는 경우") {
        ...
        When("회원가입 하면") {
            val actual = userAuthenticationService.generateTokenByRegister(request.toCommand())
    ...
    Given("이메일이 이미 존재하는 경우") {
        ...
        When("회원가입 하면") {
            Then("에러 발생한다") {
                shouldThrow<IllegalArgumentException> { userAuthenticationService.generateTokenByRegister(request.toCommand()) }
            }
        }
    }
    ...
    Given("유저정보가 존재하는 경우") {
        ...
        When("올바른 이메일로 로그인 하면") {
            val actual = userAuthenticationService.generateTokenByLogin(user.email, user.password)
        ...
        When("올바르지 않은 이메일로 로그인 하면") {
            Then("에러 발생한다") {
                shouldThrow<UnidentifiedUserException> {
                    userAuthenticationService.generateTokenByLogin("another@email.com", user.password)
                }
            }
        }
        ...
        When("올바르지 않은 비밀번호로 로그인 하면") {
            Then("에러 발생한다") {
                shouldThrow<UnidentifiedUserException> {
                    userAuthenticationService.generateTokenByLogin(user.email, Password("anotherPassword"))
                }
            }
        }
    }
    ...
    Given("리프레쉬토큰이 존재하는 경우") {
        ...
        When("리프레쉬토큰을 통해 로그인하면") {
            val actual = userAuthenticationService.generateTokenByRefreshToken(refreshToken.id)
